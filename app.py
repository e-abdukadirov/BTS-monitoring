...
# Настройка логирования для Render (чтобы видеть принты в консоли)
...
def fetch_and_sync_all():
    try:
        ...
        app.logger.error("Exception during fetch_and_sync_all: %s", e)
...
def api_refresh_data():
    try:
        ...
        app.logger.error(f">>> [API] Error during refresh: {e}")
        app.logger.error(traceback.format_exc())
